...
function addUsername(list) {
  for (let i = 0; i < list.length; i++) {
    let obj = list[i]
    let date = list[i].age - (Date.prototype.getFullYear())
    let name = (list[i].firstName.toLowerCase().concat(list[i].lastName.toLowerCase()))
    let nameNdDate = name.split('').filter((el)=>el !== '.').join('').concat(date)
    let final = nameNdDate.split('').filter((el)=>el !== '-').join('')
    obj.username = final
  }
  return list
}
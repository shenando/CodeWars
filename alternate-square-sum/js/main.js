...
function alternateSqSum(arr){
  let newArr = arr.filter((el,i)=>i %2 !== 0)
  let coolArr = arr.filter((el,i)=> i % 2 === 0)
  let nextArr = newArr.map(el=>el**2)
  let thisArr = coolArr.concat(nextArr)
  return thisArr.length > 1 ? thisArr.reduce((a,b)=>a+b) : 0
}
